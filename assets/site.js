...
if(previewJson(previousBuilder.hero?.gsap_scroll)!==previewJson(nextBuilder.hero?.gsap_scroll))setupHeroGsapScroll();
if(previewJson(previousBuilder.three_d_scene)!==previewJson(nextBuilder.three_d_scene))setupThreeDSceneV91();
...
function sfHandleV9PreviewCommand(command,data={}){if(command==='performance-profile'){sfApplyPerformanceProfile(String(data.profile||'auto'));return true;}if(command==='reduced-motion'){document.documentElement.dataset.sfForceReducedMotion=data.enabled===true?'true':'false';setupHeroGsapScroll();return true;}if(command==='motion-pause'){document.documentElement.dataset.sfMotionPaused='true';return true;}if(command==='motion-play'){document.documentElement.dataset.sfMotionPaused='false';return true;}if(command==='motion-restart'){document.documentElement.dataset.sfMotionPaused='false';document.querySelectorAll('.is-sliced-ready,[data-sf-text-motion]').forEach(node=>{node.classList.remove('is-sliced-ready','is-inview');void node.offsetWidth;node.classList.add(node.matches('[data-sf-text-motion]')?'is-inview':'is-sliced-ready');});refreshStudioFrameMotionV77(document);return true;}if(command==='timeline-scrub'){const progress=Math.max(0,Math.min(1,Number(data.progress||0)));document.documentElement.style.setProperty('--sf-preview-timeline',String(progress));scrollTo({top:progress*Math.max(0,document.documentElement.scrollHeight-innerHeight),behavior:'instant'});SF_HERO_GSAP_PROGRESS_APPLIER?.(progress);updateHeroStoryScroll(progress);sfEmitHeroBackgroundSync(progress);return true;}return false;}
