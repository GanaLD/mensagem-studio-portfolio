...
wrap.append(bindExclusiveProjectDriveFrame(directFrame));
...
fallback.append(bindExclusiveProjectDriveFrame(frame));
...
function sfGlobalMaskClip(mask={}){
const size=Math.max(0,Math.min(160,Number(mask.size??100))),x=Math.max(0,Math.min(100,Number(mask.x??50))),y=Math.max(0,Math.min(100,Number(mask.y??50)));
if(mask.type==='circle')return `circle(${size/2}% at ${x}% ${y}%)`;
if(mask.type==='ellipse')return `ellipse(${size/2}% ${Math.max(8,size*.34)}% at ${x}% ${y}%)`;
if(mask.type==='rectangle'||mask.type==='rounded')return `inset(${Math.max(0,(100-size)/2)}% ${Math.max(0,(100-size)/2)}% round ${mask.type==='rounded'?Math.max(0,Number(mask.radius||24)):0}px)`;
if(mask.type==='split'){const edge=Math.max(0,Math.min(49,(100-size)/2));return `polygon(0 0,${50-edge}% 0,${50-edge}% 100%,0 100%,0 0,100% 0,100% 100%,${50+edge}% 100%,${50+edge}% 0)`;}
if(mask.type==='slices')return 'polygon(0 0,18% 0,18% 100%,24% 100%,24% 0,47% 0,47% 100%,53% 100%,53% 0,76% 0,76% 100%,82% 100%,82% 0,100% 0,100% 100%,0 100%)';
if(mask.type==='custom_clip'&&typeof mask.clip==='string'&&mask.clip.trim())return mask.clip;
return 'none';
}
...
function setupGlobalMediaCanvasV789(){
SF_GLOBAL_CANVAS_SCROLL_CONTROLLER?.abort?.();
SF_GLOBAL_CANVAS_SCROLL_CONTROLLER=null;
document.querySelector('#sfGlobalMediaCanvas')?.remove();
const config=normalizeGlobalMediaCanvasPublic(DATA.site_builder?.global_media_canvas||{});
if(!config.enabled)return null;
const route=currentSeoRoute(),custom=currentCustomPage(),path=requestedRoutePath();
const homeLike=!path||route?.kind==='home';
const scopeAllowed=(config.scope==='site'||config.scope==='page')||(config.scope==='home'&&homeLike)||(config.scope==='category'&&route?.kind==='category')||(config.scope==='custom_page'&&Boolean(custom));
const allBlocks=publicHomeBlocks().map((block)=>normalizeSharedPublicBlock(block));
const globalBlocks=allBlocks.filter((block)=>block.visible!==false&&block.section_background_type==='global_media_canvas');
if(config.scope==='section_group'&&!globalBlocks.length)return null;
if(config.scope!=='section_group'&&!scopeAllowed)return null;
const source=sfGlobalCanvasSourceNode(config);if(!source)return null;
const root=document.createElement('div');root.id='sfGlobalMediaCanvas';root.className='sf-global-media-canvas';root.dataset.scope=config.scope;root.dataset.motionMode=config.motion_mode;root.dataset.sfGlobalCanvas='true';root.setAttribute('aria-hidden','true');root.style.setProperty('--sf-gmc-fit',config.fit);root.style.setProperty('--sf-gmc-focal-x',`${config.focal_x}%`);root.style.setProperty('--sf-gmc-focal-y',`${config.focal_y}%`);source.classList.add('sf-global-media-canvas-media');root.append(source);
(config.background_stack||[]).filter(layer=>layer&&layer.visible!==false&&layer.type!=='media_canvas').forEach(layer=>{const visual=document.createElement('i');visual.className=`sf-global-stack-layer sf-global-stack-${String(layer.type||'overlay').replace(/[^a-z0-9_-]/gi,'')}`;visual.dataset.sfGlobalStackLayer=String(layer.id||layer.type||'layer');visual.style.opacity=String(Math.max(0,Math.min(1,Number(layer.opacity??1))));visual.style.mixBlendMode=String(layer.blend_mode||'normal');root.append(visual);});
document.body.prepend(root);sfApplyGlobalCanvasFrame(root,config,{});
const blockMap=new Map(allBlocks.map(block=>[String(block.id||''),block]));
const sections=[...document.querySelectorAll('[data-home-block-id]')].map(node=>({node,block:blockMap.get(String(node.dataset.homeBlockId||''))})).filter(record=>record.block?.section_background_type==='global_media_canvas');
if(!sections.length)return root;
const activate=(record)=>{sections.forEach(item=>item.node.classList.toggle('sf-global-canvas-active',item===record));if(record)sfApplyGlobalCanvasFrame(root,config,record.block);};
let active=sections[0];activate(active);
const controller=new AbortController();SF_GLOBAL_CANVAS_SCROLL_CONTROLLER=controller;
const ratios=new Map();
if('IntersectionObserver' in window){const observer=new IntersectionObserver(entries=>{entries.forEach(entry=>ratios.set(entry.target,entry.isIntersecting?entry.intersectionRatio:0));const next=sections.slice().sort((a,b)=>(ratios.get(b.node)||0)-(ratios.get(a.node)||0))[0];if(next&&(ratios.get(next.node)||0)>0&&next!==active){active=next;if(config.motion_mode!=='section_keyframes')activate(active);else sections.forEach(item=>item.node.classList.toggle('sf-global-canvas-active',item===active));}},{threshold:[.05,.2,.4,.65],rootMargin:'-20% 0px -45% 0px'});sections.forEach(({node})=>observer.observe(node));controller.signal.addEventListener('abort',()=>observer.disconnect(),{once:true});}
const lerp=(a,b,t)=>Number(a)+(Number(b)-Number(a))*t;
const frameValues=(block={})=>({
section_global_canvas_x:Number(block.section_global_canvas_x??config.position_x),section_global_canvas_y:Number(block.section_global_canvas_y??config.position_y),section_global_canvas_scale:Number(block.section_global_canvas_scale??config.scale),section_global_canvas_rotation:Number(block.section_global_canvas_rotation??config.rotation),section_global_canvas_opacity:Number(block.section_global_canvas_opacity??config.opacity),section_global_canvas_blur:Number(block.section_global_canvas_blur??config.blur),section_global_canvas_depth:Number(block.section_global_canvas_depth??config.depth),section_global_canvas_mask:String(block.section_global_canvas_mask||config.mask.type),section_global_canvas_mask_size:Number(block.section_global_canvas_mask_size??config.mask.size),section_global_canvas_mask_radius:Number(block.section_global_canvas_mask_radius??config.mask.radius),section_global_canvas_mask_feather:Number(block.section_global_canvas_mask_feather??config.mask.feather),section_global_canvas_transition:String(block.section_global_canvas_transition||'fade')
});
const interpolateFrames=(before,after,t)=>{const a=frameValues(before),b=frameValues(after),out={};['section_global_canvas_x','section_global_canvas_y','section_global_canvas_scale','section_global_canvas_rotation','section_global_canvas_opacity','section_global_canvas_blur','section_global_canvas_depth','section_global_canvas_mask_size','section_global_canvas_mask_radius','section_global_canvas_mask_feather'].forEach(key=>out[key]=lerp(a[key],b[key],t));out.section_global_canvas_mask=t<.5?a.section_global_canvas_mask:b.section_global_canvas_mask;out.section_global_canvas_transition=t<.5?a.section_global_canvas_transition:b.section_global_canvas_transition;return out;};
if(['parallax','scroll_tracked','section_keyframes'].includes(config.motion_mode)){
let ticking=false;
const update=()=>{ticking=false;if(!root.isConnected){controller.abort();return;}if(config.motion_mode==='section_keyframes'){
const viewportCenter=innerHeight*.5;
const records=sections.map(record=>{const rect=record.node.getBoundingClientRect();return{...record,center:rect.top+rect.height*.5};}).sort((a,b)=>a.center-b.center);
let before=records[0],after=records[records.length-1];
for(let i=0;i<records.length;i+=1){if(records[i].center<=viewportCenter)before=records[i];if(records[i].center>=viewportCenter){after=records[i];break;}}
const span=Math.max(1,after.center-before.center),t=before===after?0:Math.max(0,Math.min(1,(viewportCenter-before.center)/span));const frame=interpolateFrames(before.block,after.block,t);sfApplyGlobalCanvasFrame(root,config,frame);const nearest=t<.5?before:after;if(nearest!==active){active=nearest;sections.forEach(item=>item.node.classList.toggle('sf-global-canvas-active',item===active));}
}else{const progress=(scrollY/Math.max(1,document.documentElement.scrollHeight-innerHeight))-.5;root.style.setProperty('--sf-gmc-scroll-y',`${progress*config.parallax_strength*-90}px`);}
};
window.addEventListener('scroll',()=>{if(ticking)return;ticking=true;requestAnimationFrame(update);},{passive:true,signal:controller.signal});window.addEventListener('resize',()=>{if(!ticking){ticking=true;requestAnimationFrame(update);}},{passive:true,signal:controller.signal});update();
}
controller.signal.addEventListener('abort',()=>{root.remove();},{once:true});
return root;
}
