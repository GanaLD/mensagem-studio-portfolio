...
const WHATSAPP_MARK_SVG='<svg viewBox="0 0 24 24" focusable="false" aria-hidden="true"><path d="M12 3.2a8.1 8.1 0 0 0-6.9 12.3L4 20.8l5.4-1.4A8.1 8.1 0 1 0 12 3.2Zm0 14.5c-1.2 0-2.4-.3-3.4-.9l-.4-.2-2 .5.5-1.9-.2-.4A6.4 6.4 0 1 1 12 17.7Zm3.5-4.8c-.2-.1-1.1-.6-1.3-.6-.2-.1-.3-.1-.5.1l-.6.7c-.1.2-.3.2-.5.1-1.4-.7-2.3-1.5-3-2.7-.2-.3.2-.5.6-1 .1-.2.1-.3 0-.5l-.6-1.4c-.1-.3-.3-.3-.5-.3h-.4c-.2 0-.4.1-.6.3-.6.6-.9 1.3-.9 2.1 0 .9.6 1.8.7 2 .1.1 1.3 2.1 3.3 2.9 1.3.6 2.3.8 3.1.7.9-.1 1.7-.7 2-1.4.2-.5.2-1 .1-1.1-.1-.1-.2-.1-.4-.2Z"/></svg>';
...
function createAutoplayCarouselBlock(block={}) {
  ...
  let raf=0,last=0,paused=false,finished=false;const px=Math.max(8,Math.min(160,Number(block.speed_px||32))),direction=block.direction==='right'?-1:1,loop=block.loop!==false;const tickAuto=(time)=>{if(!last)last=time;const delta=Math.min(40,time-last);last=time;if(!paused&&!finished&&viewport.scrollWidth>viewport.clientWidth){viewport.scrollLeft+=direction*px*delta/1000;const limit=loop?viewport.scrollWidth/2:viewport.scrollWidth-viewport.clientWidth;if(loop&&direction>0&&viewport.scrollLeft>=limit)viewport.scrollLeft-=limit;if(loop&&direction<0&&viewport.scrollLeft<=0)viewport.scrollLeft+=limit;if(!loop&&((direction>0&&viewport.scrollLeft>=limit-1)||(direction<0&&viewport.scrollLeft<=1)))finished=true;}raf=requestAnimationFrame(tickAuto);};raf=requestAnimationFrame(tickAuto);if(block.pause_on_hover!==false){section.addEventListener('pointerenter',()=>{paused=true;});section.addEventListener('pointerleave',()=>{paused=false;last=performance.now();});}return section;
}
...
function quoteProposalPayload(){quoteLeadFromDom();const requestId=`SF-${Date.now().toString(36).toUpperCase()}`;return {schema:'studioframe.proposal-request.v1',request_id:requestId,created_at:new Date().toISOString(),site:location.origin,lead:{...QUOTE_STATE.lead},summary:quoteSummary(),services:quoteResolvedItems().map(({category,item,line,package:pkg,reference})=>({category_id:String(category.id||''),category:String(category.title||''),service_id:String(item.id||''),service:String(item.title||''),package_id:String(pkg?.id||''),package:String(pkg?.label||''),quantity:Math.max(1,Number(line.quantity)||1),note:String(line.note||''),reference_project_id:String(reference?.project_id||reference?.id||''),reference_project:String(reference?.title||''),estimate:quoteLinePriceLabel(item,pkg,line.quantity)}))};}
...
function clearQuote(){QUOTE_STATE={version:quoteStateVersion(),updated_at:'',items:[],lead:{name_company:'',name:'',company:'',email:'',whatsapp:'',timeline:'',details:''}};saveQuoteState();}
...
function serviceCatalogTools(){const tools=document.createElement('section');tools.className='services-catalog-tools';tools.innerHTML=`<label><span>BUSCAR NO CATÁLOGO</span><input type="search" data-service-catalog-search placeholder="Ex.: vídeo, branding, campanha…" autocomplete="off"></label><div role="group" aria-label="Filtro do catálogo"><button type="button" class="is-active" data-service-catalog-mode="all">Todos</button><button type="button" data-service-catalog-mode="featured">Mais procurados</button></div><strong data-service-catalog-result></strong>`;return tools;}
function bindServiceCatalogFilters(root){const search=root.querySelector('[data-service-catalog-search]'),buttons=[...root.querySelectorAll('[data-service-catalog-mode]')],result=root.querySelector('[data-service-catalog-result]');if(!search)return;let mode='all';const apply=()=>{const query=String(search.value||'').normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase().trim();let visible=0;root.querySelectorAll('.service-area-card').forEach((category)=>{let categoryVisible=0;category.querySelectorAll('.service-price-card').forEach((card)=>{const matchText=!query||String(card.dataset.serviceSearch||'').includes(query),matchMode=mode!=='featured'||card.dataset.serviceFeatured==='1',show=matchText&&matchMode;card.hidden=!show;if(show){visible+=1;categoryVisible+=1;}});category.hidden=categoryVisible===0;});if(result)result.textContent=`${visible} ${visible===1?'serviço encontrado':'serviços encontrados'}`;};search.addEventListener('input',apply);buttons.forEach((button)=>button.addEventListener('click',()=>{mode=button.dataset.serviceCatalogMode||'all';buttons.forEach((candidate)=>candidate.classList.toggle('is-active',candidate===button));apply();}));apply();}
