...
let QUOTE_STATE={version:1,updated_at:'',items:[],lead:{name_company:'',name:'',company:'',timeline:'',details:''}};
...
function normalizeQuoteState(raw={}){const state={version:quoteStateVersion(),updated_at:String(raw?.updated_at||''),items:[],lead:{name_company:'',name:'',company:'',timeline:'',details:''}};const lead=raw?.lead&&typeof raw.lead==='object'?raw.lead:{};Object.keys(state.lead).forEach((field)=>state.lead[field]=String(lead[field]||'').slice(0,2000));if(!state.lead.name_company)state.lead.name_company=[state.lead.name,state.lead.company].filter(Boolean).join(' / ');const merged=new Map();(Array.isArray(raw?.items)?raw.items:[]).forEach((source)=>{if(!source||typeof source!=='object')return;const record=quoteServiceRecord(source.service_id);if(!record)return;let packageId=String(source.package_id||'');if(packageId&&!quotePackageRecord(record.item,packageId))packageId='';const referenceRecord=quoteProjectRecord(source.reference_project_id);const reference=referenceRecord?String(referenceRecord.project_id||referenceRecord.id||'').trim():'';const mergeKey=`${record.item.id}|${packageId}|${reference}`;const quantity=Math.max(1,Math.min(99,Number(source.quantity)||1));if(merged.has(mergeKey)){merged.get(mergeKey).quantity=Math.min(99,merged.get(mergeKey).quantity+quantity);return;}const row={id:String(source.id||newQuoteLineId()),service_id:String(record.item.id),package_id:packageId,quantity,note:String(source.note||'').slice(0,1200),reference_project_id:reference};merged.set(mergeKey,row);state.items.push(row);});return state;}
...
function clearQuote(){QUOTE_STATE={version:quoteStateVersion(),updated_at:'',items:[],lead:{name_company:'',name:'',company:'',timeline:'',details:''}};saveQuoteState();}
