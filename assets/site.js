...
return uniqueUrls([...canonical, ...source]).filter(isStudioFrameVideoPlaybackCandidate);
}
function videoSourceCandidates(item = {}) {
return canonicalDriveVideoCandidates(item);
}
...
const event={at:new Date().toISOString(),id:String(item?.id||item?.cover_media_id||''),title:String(item?.title||''),status,url:String(url||''),detail:String(detail||''),media_error_code:Number(mediaError?.code||0),media_error_message:String(mediaError?.message||''),network_state:Number(video?.networkState??-1),ready_state:Number(video?.readyState??-1),current_time:Number(video?.currentTime||0),duration:Number.isFinite(video?.duration)?Number(video.duration):null};
...
mediaDiagnostic({id:video.dataset.mediaId||'',title:video.dataset.mediaTitle||''},'failed',activeUrl,`${reason};MEDIA_ERR=${code};candidate=${sourceIndex}/${sources.length}`,video);
}
sourceReady = false;
...
video.addEventListener('canplay',()=>mediaDiagnostic({id:video.dataset.mediaId||'',title:video.dataset.mediaTitle||''},'event',activeUrl,'canplay',video));
video.addEventListener('playing',()=>mediaDiagnostic({id:video.dataset.mediaId||'',title:video.dataset.mediaTitle||''},'event',activeUrl,'playing',video));
video.addEventListener('seeked',()=>mediaDiagnostic({id:video.dataset.mediaId||'',title:video.dataset.mediaTitle||''},'event',activeUrl,'seeked',video));
...
image.alt=header.logo_alt||identity.studio_name||'Mensagem Studio'; image.classList.add('site-logo-image'); logo.append(image);
} else {
logo.dataset.logoMode='text'; logo.textContent = header.logo_label || identity.studio_name || 'MENSAGEM STUDIO';
}
}
...
const topBars=['#header','.filters','.services-jump-nav','[data-context-bar]'].map($).filter(visible).map((node)=>node.getBoundingClientRect()).filter((rect)=>rect.top<Math.min(innerHeight*.42,260));
...
document.querySelectorAll('.site-floating-control[data-floating-control]').forEach(n=>{if(!visible(n))return;(groups[n.dataset.floatingPosition||'bottom_left']||=[]).push(n)});
Object.values(groups).forEach(g=>{let x=0;g.sort((a,b)=>(rank[a.dataset.floatingControl]??9)-(rank[b.dataset.floatingControl]??9)).forEach(n=>{n.style.setProperty('--floating-stack-offset',`${Math.round(x)}px`);x+=n.getBoundingClientRect().width+8})});
if(!window.__studioframeFloatingLayoutBound){
window.__studioframeFloatingLayoutBound=1;let raf=0;const schedule=()=>{if(raf)return;raf=requestAnimationFrame(()=>{raf=0;layoutFloatingControls()})};
addEventListener('resize',schedule,{passive:true});addEventListener('scroll',schedule,{passive:true});addEventListener('hashchange',schedule);addEventListener('popstate',schedule);
new MutationObserver(schedule).observe(document.body,{subtree:true,attributes:true,attributeFilter:['hidden','class']});
}
}
...
function safeExternalHttps(value=''){try{const parsed=new URL(String(value||'').trim());return parsed.protocol==='https:'?parsed.href:''}catch(_){return ''}}
...
const frame=document.createElement('iframe');frame.src=url;frame.title=block.embed_name||block.title||'Conteúdo externo';frame.loading='lazy';frame.referrerPolicy='strict-origin-when-cross-origin';frame.sandbox='allow-forms allow-popups allow-presentation allow-same-origin allow-scripts';frame.allow='fullscreen; autoplay; encrypted-media; picture-in-picture';
const fallback=document.createElement('div');fallback.className='external-embed-fallback';fallback.innerHTML=`<strong>Este conteúdo pode bloquear a visualização incorporada.</strong><span>Abra em uma nova aba para continuar.</span>`;const open=document.createElement('a');open.href=url;open.target='_blank';open.rel='noopener';open.textContent=block.cta_text||'Abrir conteúdo externamente ↗';fallback.append(open);
let loaded=false;frame.addEventListener('load',()=>{loaded=true;stage.classList.add('is-loaded')});frame.addEventListener('error',()=>stage.classList.add('is-blocked'));setTimeout(()=>{if(!loaded)stage.classList.add('is-blocked')},6500);
...
add(false);if(block.loop!==false)add(true);viewport.append(strip);section.append(viewport);if(reduced||block.autoplay===false){viewport.classList.add('is-manual');return section}
let raf=0,last=0,paused=false,finished=false;const px=Math.max(8,Math.min(160,Number(block.speed_px||32))),direction=block.direction==='right'?-1:1,loop=block.loop!==false;const tick=(time)=>{if(!last)last=time;const delta=Math.min(40,time-last);last=time;if(!paused&&!finished&&viewport.scrollWidth>viewport.clientWidth){viewport.scrollLeft+=direction*px*delta/1000;const limit=loop?viewport.scrollWidth/2:viewport.scrollWidth-viewport.clientWidth;if(loop&&direction>0&&viewport.scrollLeft>=limit)viewport.scrollLeft-=limit;if(loop&&direction<0&&viewport.scrollLeft<=0)viewport.scrollLeft+=limit;if(!loop&&((direction>0&&viewport.scrollLeft>=limit-1)||(direction<0&&viewport.scrollLeft<=1)))finished=true}raf=requestAnimationFrame(tick)};raf=requestAnimationFrame(tick);if(block.pause_on_hover!==false){section.addEventListener('pointerenter',()=>paused=true);section.addEventListener('pointerleave',()=>{paused=false;last=performance.now()})}return section;
}
function createCinematicHeroBlock(block={}){
const media=publicMediaById(block.media_id||'');if(!media)return null;const section=document.createElement('section');section.className='modular-block block-cinematic-hero reveal';section.style.setProperty('--cinematic-height',`${Math.max(25,Math.min(100,Number(block.height_vh||50)))}vh`);section.style.setProperty('--cinematic-overlay',String(Math.max(0,Math.min(.9,Number(block.overlay??.36)))));section.style.setProperty('--cinematic-focus-x',`${Math.max(0,Math.min(100,Number(block.focal_x??50)))}%`);section.style.setProperty('--cinematic-focus-y',`${Math.max(0,Math.min(100,Number(block.focal_y??50)))}%`);
const stage=document.createElement('div');stage.className='cinematic-hero-media';if(block.parallax!==false)stage.dataset.parallax='detail';if(media.type==='video'){const video=createResilientVideo(media,{autoplay:true,muted:true,loop:true,controls:false,preload:'none',defer:true,metadataReady:false});stage.append(video);bindDeferredAutoplay(video,stage)}else stage.append(imageWithFallback(media,[media.media_url,...(media.media_candidates||[]),media.thumbnail_url,...(media.thumbnail_candidates||[])],{lazy:true,upgradeUrls:[media.media_url,...(media.media_candidates||[])]}));
const shade=document.createElement('div');shade.className='cinematic-hero-shade';const copy=blockHeading(block);copy.classList.add('cinematic-hero-copy');if(block.cta_text){const a=document.createElement('a');const externalHref=safeExternalHttps(block.cta_url),internalHref=String(block.cta_url||'').startsWith('/')?String(block.cta_url||''):'';const href=externalHref||internalHref;a.href=href||'#';a.textContent=block.cta_text;if(/^https:/i.test(a.href)){a.target='_blank';a.rel='noopener'}copy.append(a)}section.append(stage,shade,copy);return section;
}
...
const projects=builder.projects||{};
const explicitHeaderVisible=block.visible!==false;
...
const bodyNode=heading.querySelector('#projectsDescription');if(bodyNode){bodyNode.textContent=String(block.body||'');bodyNode.hidden=!bodyNode.textContent.trim();}
heading.dataset.align=['left','center','right'].includes(block.text_align)?block.text_align:'left';
}
...
function quoteLeadFromDom(){const identity=document.getElementById('quoteLeadIdentity');if(identity){QUOTE_STATE.lead.name=String(identity.value||'').slice(0,2000);QUOTE_STATE.lead.company='';}const map={timeline:'quoteLeadTimeline',details:'quoteLeadDetails'};Object.entries(map).forEach(([field,id])=>{const node=document.getElementById(id);if(node)QUOTE_STATE.lead[field]=String(node.value||'').slice(0,2000);});}
...
}, { threshold:[0,.35,.62,.85], rootMargin:'-8% 0px -8% 0px' });
observer.observe(button);
}
}
...
function normalizeQuoteIdentityUi(){
const name=document.getElementById('quoteLeadName'),company=document.getElementById('quoteLeadCompany');
if(!name||!company)return;
const label=name.closest('label'),companyLabel=company.closest('label');
name.value=[name.value,company.value].filter(Boolean).filter((value,index,list)=>list.indexOf(value)===index).join(' / ');
name.id='quoteLeadIdentity';name.autocomplete='organization';name.placeholder='Como podemos chamar você ou sua marca?';
if(label){label.classList.add('wide');const text=[...label.childNodes].find((node)=>node.nodeType===Node.TEXT_NODE);if(text)text.textContent='Nome ou marca / empresa';}
companyLabel?.remove();
}
const quoteIdentityObserver=new MutationObserver(normalizeQuoteIdentityUi);
quoteIdentityObserver.observe(document.body,{childList:true,subtree:true});
document.addEventListener('input',(event)=>{if(event.target.id!=='quoteLeadIdentity')return;QUOTE_STATE.lead.name=String(event.target.value||'').slice(0,2000);QUOTE_STATE.lead.company='';persistQuoteStateQuiet();});
normalizeQuoteIdentityUi();
