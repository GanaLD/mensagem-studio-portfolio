...
function normalizeQuoteState(raw={}){const state={version:quoteStateVersion(),updated_at:String(raw?.updated_at||''),items:[],lead:{name:'',company:'',timeline:'',details:''}};const lead=raw?.lead&&typeof raw.lead==='object'?raw.lead:{};Object.keys(state.lead).forEach((field)=>state.lead[field]=String(lead[field]||'').slice(0,2000));const merged=new Map();(Array.isArray(raw?.items)?raw.items:[]).forEach((source)=>{if(!source||typeof source!=='object')return;const record=quoteServiceRecord(source.service_id);if(!record)return;let packageId=String(source.package_id||'');if(packageId&&!quotePackageRecord(record.item,packageId))packageId='';const reference=String(source.reference_project_id||'');const mergeKey=`${record.item.id}|${packageId}|${reference}`;const quantity=Math.max(1,Math.min(99,Number(source.quantity)||1));if(merged.has(mergeKey)){merged.get(mergeKey).quantity=Math.min(99,merged.get(mergeKey).quantity+quantity);return;}const row={id:String(source.id||newQuoteLineId()),service_id:String(record.item.id),package_id:packageId,quantity,note:String(source.note||'').slice(0,1200),reference_project_id:reference};merged.set(mergeKey,row);state.items.push(row);});return state;}
