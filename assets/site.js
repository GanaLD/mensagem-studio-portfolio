...
const applyResolved=()=>{
if(window.StudioFrameThemeRuntime?.manifestFromBuilder){DATA.theme_runtime=window.StudioFrameThemeRuntime.manifestFromBuilder(builder,DATA.theme_runtime||{});return window.StudioFrameThemeRuntime.apply(DATA.theme_runtime);}
const cfg=builder.theme||{},id=String(cfg.theme_id||DATA.theme_runtime?.active_theme?.id||'studioframe-default'),variant=String(cfg.theme_variant_id||DATA.theme_runtime?.active_variant?.id||`${id}--base`);document.body.dataset.sfTheme=id;document.body.dataset.sfThemeVariant=variant.startsWith(`${id}--`)?variant:`${id}--base`;return null;
};
...
function sfHandleV9PreviewCommand(command,data={}){if(command==='performance-profile'){sfApplyPerformanceProfile(String(data.profile||'auto'));return true;}if(command==='reduced-motion'){document.documentElement.dataset.sfForceReducedMotion=data.enabled===true?'true':'false';return true;}if(command==='motion-pause'){document.documentElement.dataset.sfMotionPaused='true';return true;}if(command==='motion-play'){document.documentElement.dataset.sfMotionPaused='false';return true;}if(command==='motion-restart'){document.documentElement.dataset.sfMotionPaused='false';document.querySelectorAll('.is-sliced-ready,[data-sf-text-motion]').forEach(node=>{node.classList.remove('is-sliced-ready','is-inview');void node.offsetWidth;node.classList.add(node.matches('[data-sf-text-motion]')?'is-inview':'is-sliced-ready');});refreshStudioFrameMotionV77(document);return true;}if(command==='timeline-scrub'){const progress=Math.max(0,Math.min(1,Number(data.progress||0)));document.documentElement.style.setProperty('--sf-preview-timeline',String(progress));scrollTo({top:progress*Math.max(0,document.documentElement.scrollHeight-innerHeight),behavior:'instant'});updateHeroStoryScroll(progress);sfEmitHeroBackgroundSync(progress);return true;}return false;}
load().then(() => {
...
const V9_PHASE_E_3D_ASSETS=Object.freeze({'oryzo-controller':{id:'oryzo-controller',engine:'glb',bundled_path:'3d/oryzo-controller.glb.gz',compression:'gzip',runtime_ready:true}});
