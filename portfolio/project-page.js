(() => {
  ...
  function caseStudyHTML() {
    ...
    if (!c) return '';
    const card = (label, text, wide=false) => text
      ? `<article class="case-study-card${wide ? ' case-study-card-wide' : ''}"><small>${esc(label)}</small><p>${esc(text)}</p></article>`
      : '';
    ...
    return `<section class="case-study-section"><div class="wrap">
      <div class="case-study-head">
        <div><div class="eyebrow">CASE DO PROJETO</div><h2>Do briefing à entrega.</h2></div>
        <p>${esc(c.intro || '')}</p>
      </div>
      <div class="case-study-grid">
        ${card('Contexto', c.context, true)}
        ${card('Objetivo', c.objective)}
        ${card('Solução visual', c.solution)}
        ${card('Processo', c.process, true)}
