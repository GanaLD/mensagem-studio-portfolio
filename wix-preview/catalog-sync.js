...
(() => {
  ...
  style.textContent = `
    ...
    .ms3d-shell{position:relative;margin-top:8px;min-height:560px;display:grid;align-items:center;perspective:1700px;perspective-origin:50% 44%;overflow:hidden;touch-action:pan-y;user-select:none}
    .ms3d-shell:before{content:"";position:absolute;left:50%;bottom:66px;width:min(72vw,980px);height:160px;transform:translateX(-50%) rotateX(78deg);border-radius:50%;background:radial-gradient(ellipse,rgba(201,255,54,.12),rgba(201,255,54,.025) 48%,transparent 72%);filter:blur(16px);pointer-events:none}
    ...
    .ms3d-stage{position:relative;height:470px;transform-style:preserve-3d;display:grid;place-items:center;cursor:grab}
    ...
    .ms3d-card{--focus:0;position:absolute;left:50%;top:50%;width:clamp(220px,18vw,290px);height:360px;margin-left:calc(clamp(220px,18vw,290px)/-2);margin-top:-180px;border:1px solid rgba(255,255,255,calc(.12 + var(--focus)*.38));border-radius:26px;background:linear-gradient(145deg,rgba(255,255,255,calc(.045 + var(--focus)*.055)),rgba(255,255,255,.014) 42%,rgba(201,255,54,calc(.012 + var(--focus)*.045)));backdrop-filter:blur(20px) saturate(1.22);box-shadow:0 30px 90px rgba(0,0,0,.34),0 0 calc(8px + var(--focus)*34px) rgba(201,255,54,calc(var(--focus)*.16)),inset 0 1px 0 rgba(255,255,255,.08);overflow:hidden;transform-style:preserve-3d;transition:border-color .18s linear,box-shadow .18s linear,filter .18s linear,opacity .18s linear;will-change:transform,opacity,filter;isolation:isolate}
    ...
    .ms3d-inner{height:100%;padding:26px;display:flex;flex-direction:column;transform:translateZ(34px)}
    ...
    .ms3d-arrow{width:36px;height:36px;border-radius:50%;display:grid;place-items:center;border:1px solid rgba(255,255,255,.16);background:rgba(255,255,255,.035);transition:.2s ease}
    ...
    .ms3d-copy h3{font-size:clamp(27px,2.3vw,39px);line-height:.96;letter-spacing:-.045em;margin:0 0 13px;max-width:10ch}
    .ms3d-copy p{margin:0;color:var(--muted);line-height:1.5;font-size:13px;max-width:31ch}
    .ms3d-foot{margin-top:22px;padding-top:15px;border-top:1px solid rgba(255,255,255,.1);display:flex;justify-content:space-between;gap:16px;align-items:center;font-size:9px;letter-spacing:.12em;text-transform:uppercase;color:#d9ddd0}
    .ms3d-foot b{color:var(--lime);font-weight:700}

    .ms3d-controls{position:absolute;left:50%;bottom:18px;z-index:30;transform:translateX(-50%);display:flex;align-items:center;gap:12px;padding:8px 10px;border:1px solid rgba(255,255,255,.10);border-radius:999px;background:rgba(8,9,7,.52);backdrop-filter:blur(12px)}
    ...
    .ms3d-hint{position:absolute;left:50%;bottom:0;transform:translate(-50%,100%);color:#6f766b;font-size:9px;letter-spacing:.14em;text-transform:uppercase;white-space:nowrap}

    @media(max-width:1024px){
      .ms3d-shell{min-height:520px;perspective:1450px}
      .ms3d-stage{height:430px}
      .ms3d-card{width:240px;height:330px;margin-left:-120px;margin-top:-165px}
      .ms3d-inner{padding:23px}
    }
    @media(max-width:760px){
      .ms3d-shell{min-height:470px;margin-left:calc(var(--pad)*-1);margin-right:calc(var(--pad)*-1);perspective:1100px}
      .ms3d-shell:after{background:linear-gradient(90deg,#070806 0,transparent 5%,transparent 95%,#070806 100%)}
      .ms3d-stage{height:390px}
      .ms3d-card{width:210px;height:300px;margin-left:-105px;margin-top:-150px;border-radius:22px}
      .ms3d-inner{padding:20px}
      .ms3d-copy h3{font-size:28px}
      .ms3d-copy p{font-size:12px}
      .ms3d-controls{bottom:4px}
    }
  ...
  function computeRadius(){
    ...
    if (w < 520) radius = Math.max(250, w * .62);
    else if (w < 900) radius = Math.max(360, w * .54);
    else radius = Math.min(650, Math.max(500, w * .42));
  }
  ...
  function render(){
    ...
    cards.forEach((card,i) => {
      ...
      const lift = Math.sin(Math.abs(rad)) * -18;
  ...
  cards.forEach((card,i) => {
    card.addEventListener('click', e => {
      ...
      const angle = Math.abs(signed(i * step + rotation));
      if (angle > step * .72){
        e.preventDefault();
        snapTo(i);
      }
  ...
  function tick(now){
    ...
    if (!dragging){
      ...
      if (!reduceMotion && idleFor > 1200){
        targetRotation -= dt * .0032; // ~0.19 deg/s: slow continuous orbit
      }
